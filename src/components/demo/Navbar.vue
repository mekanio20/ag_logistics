    ...
    <div class="sm:container w-full">
        ...
        <header class="sm:flex hidden lg:flex-row flex-col lg:space-y-0 space-y-4 items-center justify-between py-5">
            ...
            <div class="flex sm:flex-row flex-col items-center space-x-4">
                <div class="flex items-center space-x-2">
                    <phone />
                    <p class="font-manjari font-normal text-sm text-m_gray-200">+993 12 96-49-97</p>
                </div>
                <div class="flex items-center space-x-2">
                    <mail />
                    <p class="font-manjari font-normal text-sm text-m_gray-200">mail@logistics.tm </p>
                </div>
                <div class="flex items-center space-x-2">
                    <pin />
                    <p class="font-manjari font-normal text-sm text-m_gray-200">744000, Ashgabat, Seyitnazar Seydi 70/2
                    </p>
                </div>
            </div>
            <div class="flex items-center space-x-6">
                ...
                <div class="flex items-center space-x-2">
                    <div v-for="item in icons" :key="item.id" @mouseenter="hoveredIcon = item.id"
                        ...
                        class="rounded-full cursor-pointer w-[30px] h-[30px] flex flex-col items-center justify-center bg-m_gray-400 hover:bg-m_blue-100 hover:-mt-1 duration-300">
                        <a :href="item.url">
                ...
                <div class="flex items-center space-x-2 cursor-pointer select-none relative" @click="toogleLang">
                    ...
                    <p class="font-manjari font-normal text-sm text-m_gray-200">{{ activeLang }}</p>
        ...
        <nav class="flex items-center select-none">
            ...
            <!-- <router-link to="/contact"
                class="before:ease relative lg:text-base text-sm py-8 px-10 overflow-hidden bg-m_blue-400 text-white transition-all before:absolute before:right-0 before:top-0 before:h-40 before:w-6 before:translate-x-12 before:rotate-6 before:bg-white before:opacity-10 before:duration-700 hover:shadow-m_blue-100 hover:before:-translate-x-40">
                <span relative="relative z-10">Contact us</span>
            </router-link> -->
            <router-link to="/contact"
                class="sm:block hidden font-manjari font-normal lg:text-base text-sm bg-m_blue-400 hover:bg-red-700 duration-300 text-white py-8 px-10">
                Contact us
            </router-link>
        </nav>
    </div>
    ...
    <div class="fixed top-0 left-0 w-full h-full bg-white z-10 transition-all duration-500 ease-in-out"
        ...
        <div v-show="isOpen" class="p-8">
            ...
            <div class="w-full flex flex-col items-center text-center">
                <router-link v-for="item in pages" :key="item.id" :to="item.url"
                    class="w-full font-sf_pro font-medium lg:text-lg text-base cursor-pointer text-black py-4 border-b border-gray-200 hover:text-m_blue-100 duration-300"
                    :class="{ 'text-m_blue-100': $route.path === item.url }">
                    {{ item.name }}
                </router-link>
                <router-link to="/contact"
                    :class="{ 'text-m_blue-100': $route.path === '/contact' }"
                    class="w-full font-sf_pro font-medium lg:text-lg text-base cursor-pointer text-black py-4 border-b border-gray-200 hover:text-m_blue-100 duration-300">
                    Contact us
                </router-link>
            </div>
